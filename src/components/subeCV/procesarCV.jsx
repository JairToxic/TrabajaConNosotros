...
const SubirCurriculum = ({processId}) => {
  ...
  const [preexistingProcess, setPreexistingProcess]=useState(null)
  ...
  const handleCreateClick = async () => {
    try {
      const response = await fetch('http://51.222.110.107:5012/applicant/', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': '7zXnBjF5PBl7EzG/WhATQw==',
          'Token': session.user.data.token // Ensure `session.user.data.token` is valid
        },
        body: JSON.stringify({
          "cv_data": {
            "certificaciones": [],
      ...
      console.log('Response Data Id:', data.applicant_data_id);
      window.location.href = `/editar-cv/${data.applicant_data_id}`;
    } catch (error) {
    ...
    const fetchUserData = async () => {
      if (session) {
        try {
          ...
          const employee = await getEmployeeByUserID(session, user.id);
          setType(employee.status === 500 ? "External" : "Internal");

          const response = await fetch(
            `http://51.222.110.107:5012/applicant/search?user_id=${user.id}&process_id=${processId}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          console.log("Applicant Search Status:", response.status); // Debugging

          if (response.status === 200) {
            setPreexistingProcess(true);
          }
          else{
            setPreexistingProcess(false)
          }
        } catch (error) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      window.location.href = `/editar-cv/${res.data.applicant_data_id}`;
    } catch (err) {
  ...
  return (
    <>
      {preexistingProcess === null ? (
        // Show a loading spinner when preexistingProcess is null
        <div
          style={{
            display: "flex",
        ...
        >
          <div style={{ marginBottom: "20px" }}>
            <FaCheckCircle style={{ color: '#21498e' }} size={200}/>
          </div>
          <h2
            style={{
              fontSize: "1.5rem",
